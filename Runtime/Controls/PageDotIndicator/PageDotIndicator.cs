...
namespace UnityUIToolkit.Extensions
{
    ...
    /// </summary>
    [UxmlElement]
    public partial class PageDotIndicator : VisualElement
    {
        ...
        private Color pendingColor = new Color(0.8f, 0.8f, 0.8f, 1f);
        ...
        [UxmlAttribute("current-page")]
        public int CurrentPage
        {
            get => currentPage;
            set => SetProgress(value, totalPages);
        }

        [UxmlAttribute("total-pages")]
        public int TotalPages
        {
            get => totalPages;
            set => SetProgress(currentPage, value);
        }

        public float NormalizedProgress => totalPages > 0 ? (float)(currentPage + 1) / totalPages : 0f;

        [UxmlAttribute("completed-color")]
        ...
        {
            CompletedColor = ParseColor(colorHex);
        }

        public void SetPendingColor(string colorHex)
        {
            PendingColor = ParseColor(colorHex);
        }

        public void SetColors(string completedColorHex, string pendingColorHex)
        {
            completedColor = ParseColor(completedColorHex);
            pendingColor = ParseColor(pendingColorHex);
            overrideColors = true;
            UpdateDots();
        }
        ...
        private void UpdateDots()
        {
            ...
            for (int i = 0; i < dots.Count; i++)
            {
                ...
                if (overrideColors)
                {
                    dots[i].style.backgroundColor = isCompleted ? completedColor : pendingColor;
                }
            }
        }
